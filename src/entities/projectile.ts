...
export class Projectile {
  ...
  public checkCollisions(enemies: Enemy[], player: Player, particles: Particle[]): boolean {
    // Проверка выхода за границы карты или смерти снаряда
    if (this.x < 0 || this.x > 4000 || this.y < 0 || this.y > 4000 || this.isDead()) {
      return true;
    }
    ...
    if (this.color === "yellow") {
      for (let j = enemies.length - 1; j >= 0; j--) {
        ...
        if (Math.hypot(this.x - enemy.x, this.y - enemy.y) < this.size + enemy.size) {
          enemy.hp -= 20;
